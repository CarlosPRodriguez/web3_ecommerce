...
export const addItem2Cart =
	(itemId: number, qty: number = 1, callToOrder: boolean = true): AppThunk =>
	async (dispatch, getState) => {
		try {
			...
			const response = await apiClient.cart.addItemToCart(cartId, itemId, qty);
			const { product, actionRequired, cartTotal, added } = response;
			...
			if (actionRequired === "chooseVariant" && product) {
				...
				if (callToOrder && added) {
					dispatch(showCall2Order(added));
				}
			}
		} catch (err) {
			dispatch(setCartSubmitting(false)); // Ensure we reset the submitting state
			dispatch(showErrorAlert("An error occurred while adding item to cart")); // Show user-friendly error
			console.error(err);
		}
	};
